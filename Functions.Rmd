...
segments(x0 = 25, y0 = 0, y1 = 2.5, x1 = 25, col = 'red')
...
plot(cleanData$Age, cleanData$BMI)

abline(h = 25, col = 'red')

plot(bmi, homa)

plot(fasting insulin, homa)


```

```{r}
plot(cleanData$Age, cleanData$HOMA)
cor.test(age_vals, HOMA_vals)
```
```{r}
plot(cleanData$Age, cleanData$BMI)
...
```{r}
plot(cleanData$BMI, cleanData$HOMA)
...
```{r}
plot(cleanData$`Fasting Insulin`, cleanData$HOMA)
